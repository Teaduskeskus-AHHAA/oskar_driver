...
OdomPlugin::OdomPlugin(BoardComms* comms, std::string name) : Plugin(comms, name)
{
  ...
  timer_ = nh_.createTimer(ros::Duration(0.01), &OdomPlugin::fake_odom, this);
}
...
void OdomPlugin::reset()
{
  ...
  odom_transform_.transform.rotation.w = 1;
}
...
void OdomPlugin::fake_odom(const ros::TimerEvent& event)
{
  if (!this->comms_->connected())
  {
    reset();
    if (odom_pub_.getNumSubscribers() > 0)
    {
